...
class ProfileSearch:
  def __init__(self, battleTag):
    self.playerNickname = battleTag.replace('#', '-', 1)
    ...
    try:
      ...
      self.notFoundPlayerStat()

    except:
      print('Error: Not Found User => ' + self.battleTag)
      self.notFoundUserError()

  def notFoundPlayerStat(self):
  ...
  def parseJson(self):
    ...
    if not self.profile:
      ...
      self.embed.add_field(name='경쟁전', value='승리: ' + str(self.competitiveWin) + "판\n" + "패배: " + str(
        self.competitiveLost) + "판\n" + "무승부: " + str(self.competitiveDraw) + "판\n" + "플레이 판 수: " + str(
        self.competitivePlayed) + "판\n" + "승률: " + str(self.competitiveWin_rate) + "%\n" + "플레이 시간" + str(
        self.competitivePlayTime), inline=False)
...
class StatsSearch:
    ...
    if self.Game == '빠른대전':
      ...
      try:
        for i in range(0, 3):
          ...
          self.competitiveWeapon_accuracyHero[i] = self.j['stats']['top_heroes']['competitive']['weapon_accuracy'][i][
            'hero']
          self.competitiveWeapon_accuracy[i] = self.j['stats']['top_heroes']['competitive']['weapon_accuracy'][i][
            'weapon_accuracy']
          self.competitiveKDAHero[i] = self.j['stats']['top_heroes']['competitive']['eliminations_per_life'][i]['hero']
          self.competitiveKDA[i] = self.j['stats']['top_heroes']['competitive']['eliminations_per_life'][i][
  ...
  def quickParseJson(self):
    for i in range(0, 3):
      self.embed.add_field(name="플레이 시간 top " + str(i + 1),
                           value=self.quickPlayedHero[i] + ": " + str(self.quickPlayedTime[i]), inline=True)

    for i in range(0, 3):
      self.embed.add_field(name="승리한 판 top " + str(i + 1),
                           value=self.quickWonHero[i] + ": " + str(self.quickWonGames[i]) + "판", inline=True)

    for i in range(0, 3):
      self.embed.add_field(name="명중률 top " + str(i + 1),
                           value=self.quickWeapon_accuracyHero[i] + ": " + str(self.quickWeapon_accuracy[i]),
                           inline=True)
  ...
  def competitiveParseJson(self):
    for i in range(0, 3):
      self.embed.add_field(name="플레이 시간 top " + str(i + 1),
                           value=self.competitivePlayedHero[i] + ": " + str(self.competitivePlayedTime[i]), inline=True)
    
    for i in range(0, 3):
      self.embed.add_field(name="승률 top " + str(i + 1),
                           value=self.competitiveWin_rateHero[i] + ": " + str(self.competitiveWin_rateGames[i]),
                           inline=True)
    
    for i in range(0, 3):
      self.embed.add_field(name="K/DA top " + str(i + 1),
                           value=self.competitiveKDAHero[i] + ": " + str(self.competitiveKDA[i]), inline=True)
    
    for i in range(0, 3):
      self.embed.add_field(name="명중률 top " + str(i + 1), value=self.competitiveWeapon_accuracyHero[i] + ": " +
                                                               str(self.competitiveWeapon_accuracy[i]), inline=True)
